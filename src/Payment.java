   ...
   private void generator() {
    ...
    private void setCombo() {
        ...
        try {
            ResultSet rst = DBConnection.getDBConnection().createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_UPDATABLE).executeQuery("SELECT category FROM users WHERE category = 'Booking Clerk'");
            while (rst.next()) {
                combo3.addItem(rst.getString(1));
            }
        } catch (Exception n) {
            n.printStackTrace();
        }
    ...
    private void setcbr() {
        ...
        try {
            ResultSet rst = DBConnection.getDBConnection().createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_UPDATABLE).executeQuery("SELECT * FROM Passenger where Booked_Status='Booked' and Pay_Status IS NULL");
            while (rst.next()) {
