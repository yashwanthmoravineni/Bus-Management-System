...
//		button2.setEnabled(true);
    ...
    private void setCbx() {
        try {
            ResultSet rst = DBConnection.getDBConnection().createStatement(
                    ...
                    ResultSet.CONCUR_UPDATABLE).executeQuery("SELECT Bus_Reg,Dept_Time From Schedules where Route_Name='" + combo7.getSelectedItem() + "'");
    ...
    private void setCombo() {
        try {
            ResultSet rst = DBConnection.getDBConnection().createStatement(
                    ResultSet.TYPE_SCROLL_INSENSITIVE,
                    ResultSet.CONCUR_UPDATABLE).executeQuery("SELECT Pass_No,Pass_Name FROM Passenger where Booked_Status IS NULL");
